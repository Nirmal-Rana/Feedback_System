...
namespace CollegeIssueManagement.Controllers
{
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> Issues(
            string type = "all", string status = "all")
        {
            if (HttpContext.Session.GetString("AdminUsername") == null)
                return RedirectToAction("Login");
            ...
            if (type != "all" &&
                Enum.TryParse<IssueType>(type, out IssueType parsedType))
                filtered = filtered.Where(i => i.Type == parsedType);

            if (status != "all" &&
                Enum.TryParse<IssueStatus>(status, out IssueStatus parsedStatus))
        ...
        public async Task<IActionResult> UpdateIssueStatus(
            int id, string status, string remarks)
        {
            if (HttpContext.Session.GetString("AdminUsername") == null)
            {
            ...
            _context.Notifications.Add(new Notification
            {
                ...
                Message = "Your " + issue.Type + " issue changed from " +
            ...
            try
            {
                ...
                string emailBody =
                    ...
                    "<p>Your issue <strong>#" + issue.Id + " (" + issue.Type +
                    ...
                    "<tr>" +
                    "<td style='padding:10px;font-weight:bold;'>New Status</td>" +
                    "<td style='padding:10px;font-weight:bold;color:#667eea;'>" +
                    newStatus + "</td></tr>" +
                    remarksRow +
                    "</table>" +
                    "<p style='color:#888;font-size:12px;'>" +
                    "— Texas College Issue Management System</p>" +
                    "</body></html>";

                await _emailService.SendEmailAsync(
                    issue.StudentEmail,
                    "Issue Update #" + issue.Id + ": " + issue.Type +
                    " — " + newStatus,
                    emailBody);
            }
            catch (Exception ex)
            ...
            if (Request.Headers["X-Requested-With"] == "XMLHttpRequest")
            {
                ...
                {
                    success = true,
                    message = "Status updated from " + oldStatus +
                                " to " + newStatus + ".",
            ...
            TempData["Success"] = "Issue status updated from " + oldStatus +
        ...
        public async Task<IActionResult> GenerateReport(
            string fromDate, string toDate, string issueType)
        {
            if (HttpContext.Session.GetString("AdminUsername") == null)
                return RedirectToAction("Login");
            ...
            if (!string.IsNullOrEmpty(issueType) && issueType != "all" &&
                Enum.TryParse<IssueType>(issueType, out IssueType parsedType))
